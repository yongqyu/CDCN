...
class Spoofing_train(Dataset):
    ...
    def get_single_image_x(self, image_path, map_path, videoname):
        ...
        frames_total = [name for name in os.listdir(map_path) if os.path.isfile(os.path.join(map_path, name))]
        image_name = np.random.choice(frames_total, 1)[0]
        ...
        # gray-map
        map_path = os.path.join(map_path, image_name)
